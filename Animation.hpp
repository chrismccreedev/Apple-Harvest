...
public:
	...
	bool loop;//показывает, зациклена ли анимация, для того чтобы ее можно было остановить.
	bool flip;//показывает, отражена ли картинка анимации (при движении в другую сторону)
	bool isPlaying;//проигрывается или нет
	Sprite sprite;
	...
	void tick(float time)
	{
		...
		if (currentFrame >= frames.size()) { currentFrame = 0 /*frames.size()*/;
		                                    if (!loop) {isPlaying=false; return;}
		                                  }

		int i = currentFrame;
		//cout<<"ANIM:"<<currentFrame<<","<<frames.size()<<endl;
